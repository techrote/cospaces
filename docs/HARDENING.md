...
- **Real Codespace smoke:** **pending**. It was not executed during issue #18 because no existing disposable Codespace was explicitly selected for live use.
...
- workspace zero/one/multiple selection and ambiguity behavior;
- T2 transport, timeout, and remote-failure distinctions;
- checkpoint corruption/history/path-safety behavior;
...
2. run a bounded `git status --short` through T2;
3. run a temporary one-check T4 verification plan through the same Codespace;
...
- T4 verification succeeds and returns a structured verification ID;
